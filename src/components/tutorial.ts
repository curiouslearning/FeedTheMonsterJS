...
export class Tutorial {
  ...
  updateTargetStonePositions(targetStonePosition: number[]) {
    ...
    this.monsterStoneDifference = Math.sqrt(
  ...
  draw(deltaTime: number, img: CanvasImageSource, imageSize: number) {
    if (this.imagesLoaded && !this.endTutorial && this.shouldPlayTutorial()) {
      ...
      let monsterStoneDifferenceInPercentage =
        (100 * distance) / this.monsterStoneDifference;
      if (monsterStoneDifferenceInPercentage < 15) {
        if (monsterStoneDifferenceInPercentage > 1) {
          this.createHandScaleAnimation(
            deltaTime,
            this.endx,
            this.endy + 30,
            true
        ...
        drawImageOnCanvas(
          ...
          this.x + 15,
          this.y + 10
        ); //draws the hand stone drag animation!
      }
    }
  }
  createHandScaleAnimation(
    deltaTime: number,
    offsetX: number,
    offsetY: number,
    shouldCreateRipple: boolean
  ) {
    this.totalTime += Math.floor(deltaTime);
    const transitionDuration = 500;
    const scaleFactor = this.sinusoidalInterpolation(
      this.totalTime,
      1,
      1.5,
      transitionDuration
    ...
    shouldCreateRipple
      ...
      : this.drawRipple(
          ...
          offsetY + this.tutorialImg.height / 2,
          false
        );
  }
  ...
  clickOnMonsterTutorial(deltaTime) {
    if (this.shouldPlayMonsterClickTutorialAnimation()) {
      ...
      if (this.totalTime < transitionDuration / 2) {
        currentOffsetY =
          topPosition +
          (this.totalTime / (transitionDuration / 2)) *
            (bottomPosition - topPosition);
        this.drawRipple(
          offsetX,
          this.height / 1.9 + this.tutorialImg.height / 1.5,
          true
        );
      } else {
        currentOffsetY =
