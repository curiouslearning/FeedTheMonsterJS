...
export class PromptText extends EventManager implements PromptTextInterface {
  ...
  public scaleFactor: number = 0.0005;
  ...
  public getPromptAudioUrl = (): string => {
    return Utils.getConvertedDevProdURL(
  ...
  playSound = () => {
    if (this.isAppForeground) {
      this.audioPlayer.playPromptAudio(
        Utils.getConvertedDevProdURL(this.currentPuzzleData.prompt.promptAudio)
      );
    }
  };
  ...
  setCurrrentPuzzleData(data: any) {
  ...
  drawRTLLang() {
    ...
    if (this.levelData.levelMeta.levelType === "LetterInWord") {
      if (this.levelData.levelMeta.protoType === "Visible") {
        const letterInWord = this.currentPromptText.replace(
      ...
      if (this.levelData.levelMeta.protoType === "Visible") {
        x -= this.context.measureText(this.currentPromptText).width * 0.5;
        for (let i = this.targetStones.length - 1; i >= 0; i--) {
          this.context.fillStyle =
            this.droppedStones > i || this.droppedStones === undefined
              ? "black"
              : "red";
          this.context.fillText(this.targetStones[i], x, y);
          x += this.context.measureText(this.targetStones[i]).width + 5;
        }
      } else {
        this.context.drawImage(
          this.promptPlayButton,
  ...
  drawOthers() {
    ...
    for (let i = 0; i < promptTextLetters.length; i++) {
        ...
        case "LetterInWord": {
          if (this.levelData.levelMeta.protoType === "Visible") {
            if (letterHighlight.includes(promptTextLetters[i])) {
              letterHighlight = letterHighlight.slice(1);
              this.context.fillStyle = "red";
      ...
      currentWordWidth =
        (this.context.measureText(promptTextLetters[i]).width +
          this.context.measureText(promptTextLetters[i + 1]).width) /
        2;
  ...
  draw(deltaTime: number) {
    this.updateScaling();
    this.time =
      deltaTime < 17 ? this.time + Math.floor(deltaTime) : this.time + 16;
    ...
    if (!this.isStoneDropped) {
      ...
      this.context.fillStyle = "black";
      this.rightToLeft ? this.drawRTLLang() : this.drawOthers();
    }
  }
  ...
  calculateFont(): number {
    return this.promptImageWidth / this.currentPromptText.length > 35
      ? 35
  ...
  async loadImages() {
    const image1Promise = this.loadImage(
      this.prompt_image,
      "./assets/images/promptTextBg.png"
    );
    const image2Promise = this.loadImage(
      this.promptPlayButton,
      "./assets/images/promptPlayButton.png"
    );
